...
struct PDFSummarizerView: View {
    ...
    @State private var overlayImages: [CGImage] = []
        ...
        ScrollView {
            VStack(spacing: 20) {
                ...
                VStack(spacing: 12) {
                    ...
                    Button(action: {
                        Task {
                            await analyzeAssetPDF(named: "MS_test_report")
                        }
                    }) {
                        HStack {
                        ...
                        ScrollView(.horizontal, showsIndicators: true) {
                            HStack(spacing: 12) {
                                ForEach(overlayImages.indices, id: \.self) { index in
                                    Image(decorative: overlayImages[index], scale: 1.0)
                                        .resizable()
    ...
    private func analyzeDocument(from url: URL) async {
        ...
        do {
            ...
            let rendered = visuals.compactMap {
    ...
    private func analyzeAssetPDF(named filename: String) async {
        ...
        do {
            ...
            do {
                let result = try await FoundationSummaryClient.summarizeExistingPDF(named: filename)
                await MainActor.run {
                    summary = result
                    isProcessing = false
                }
